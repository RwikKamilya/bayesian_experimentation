...
Outputs:
  - results_coco_transformer_vs_qlogei.csv
      per-evaluation best feasible value for each method.
  - results_coco_transformer_vs_qlogei_summary.csv
      one row per method/problem/repetition, final best feasible value.
"""
...
class NextConfigTransformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, attn_mask: torch.Tensor) -> torch.Tensor:
        ...
        idx = lengths.view(B, 1, 1).expand(-1, 1, enc_out.size(-1))
        last_hidden = enc_out.gather(1, idx).squeeze(1)

        pred_x = self.head(last_hidden)
        return pred_x
...
class TransformerOptimizer:
    ...
    y and c must be normalised with the same stats as during training.
    """
    ...
    def __init__(self, dim: int, ckpt_path: Path):
        ...
        self.y_mean = float(ckpt.get("y_mean", 0.0))
        self.y_std = float(ckpt.get("y_std", 1.0))
        self.c_mean = float(ckpt.get("c_mean", 0.0))
        self.c_std = float(ckpt.get("c_std", 1.0))
    ...
    def _normalise_yc(self, y: np.ndarray, c: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        y_norm = (y - self.y_mean) / (self.y_std + 1e-8)
        c_norm = (c - self.c_mean) / (self.c_std + 1e-8)
        y_norm = np.clip(y_norm, -5.0, 5.0)
        c_norm = np.clip(c_norm, -5.0, 5.0)
    ...
    def suggest(
        ...
        y_seq_raw = np.asarray(f_hist[-L:], dtype=np.float32)   # [L]
        c_seq_raw = np.asarray(c_hist[-L:], dtype=np.float32)   # [L]

        # Normalise y and c as in training
        y_seq, c_seq = self._normalise_yc(y_seq_raw, c_seq_raw)
        ...
        attn_mask_t = torch.tensor(attn_mask, dtype=torch.float32, device=DEVICE).unsqueeze(0)
        ...
        for fid in FUNCTIONS:
            for inst in INSTANCES:
                ...
                for rep in range(REPETITIONS):
                    ...
                    # ---- Periodically save CSVs so progress is not lost ----
